...
class DummyNetwork(NeuralNetwork):
    ...
    def step(
        ...
        aux_triangle = 2*np.abs(
            2*(
                time - np.arange(self.n_body_joints)/self.n_body_joints
                - np.floor(time - np.arange(self.n_body_joints) /
                           self.n_body_joints + 0.5)
            )
        ) - 1
        act_left = 1+aux_triangle
        act_right = 1-aux_triangle
...
class DummyController(PolymanderController):
    """DummyController"""
    ...
    def __init__(self,
        ...
        self.left_leg_idx = slice(
            2*self.n_body_joints, 2*self.n_body_joints + 2*self.n_leg_joints, 2)
        self.right_leg_idx = slice(
            2*self.n_body_joints+1, 2*self.n_body_joints + 2*self.n_leg_joints+1, 2)

        animat_network = DummyNetwork(data=animat_data,
